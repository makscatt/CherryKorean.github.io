...
Конвейер песни делят два агента (граница уточнена боссом 2026-07-21):

- **makscut-агент** (`studio/engines/makscut/`, runbook — `АЛГОРИТМ: караоке-песня`
  в его `CLAUDE.md`) готовит **готовый результат С ПЕРЕВОДАМИ, разложенный прямо в
  это репо**: нарезку фраз + полный клип в `video/song<N>/` (webm + `mp4/`),
  `karaoke_lyrics.json`, и **полностью собранный** `videothemes/json/videothemeNN.json`
  (`karaoke.lyrics` и `b1` с ru+en, `isSong`, `isNewSong:true`, `b2..b5:[]`,
  `b1` уже отсортирован по порядку песни). Это шаги **1–4.1** ниже. При передаче он
  называет, какие `song<N>`/`NN` занял.
- **mini-app-агент** (этот проект) **довершает интеграцию**: шаги **5–7** —
  снятие `isNewSong` с прошлой песни (шаг 5), регистрация в `config.json` (секция
  `songs`, шаг 6), `rebuild_config.py` + деплой (шаг 7). Нарезку/тайминги/переводы/
  сшивку/сортировку `b1` НЕ трогает — они уже готовы.
...
### 2. Тайминги строк — РУЧНОЙ karaoke_timer (основной)

**Основной путь — руками** (вывод босса 2026-07-21: авто под тайминги — слабый
инструмент, Whisper по пению склеивает фразы и пропускает быстрые куски; проще и
точнее проставить вручную). Тулза:
`pythonw projects/studio/engines/makscut/karaoke/karaoke_timer.pyw`
...
**Авто (`align_lyrics.py`) — опционально/слабый**, не основной. Даёт черновые `t`
(Groq Whisper + forced alignment) с точным `end` по последнему слову, но по пению
врёт; годится максимум как черновик под ручную докрутку. Как встроить ручной timer
в новый пайплайн — открытый вопрос (см. makscut `reports/tasks.json`).
