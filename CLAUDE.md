...
Рядом лежит `SONGS.md` — алгоритм добавления песни (караоке-видеотемы):
тайминги, переводы, сшивка JSON, регистрация в config. Читай его при любых
вопросах про песни/караоке.

Общайся кратко, отвечай строго по сути вопроса. Всё что надо — я спрошу сам.

# Входящие от makscut-агента — `reports/agent_inbox/`

Makscut-агент (`studio/engines/makscut`) готовит видео-контент (караоке-песни,
нарезку корейских фраз из дорам) и раскладывает готовые файлы **прямо в этот репо**
(`video/…`, `videothemes/json/…`, обложки), а знание о созданном передаёт
**файлом-сигналом** в `reports/agent_inbox/`. Он НЕ трогает `config.json` и деплой —
интеграция в систему это ТВОЯ зона.

**При старте сессии (и по запросу «проверь входящие») проверяй
`reports/agent_inbox/` на новые `*.md` со `status: pending`.** Для каждого сигнала:

- **`type: song`** → интегрируй по `SONGS.md`: сверь файлы → добавь запись в
  `config.json` секция `songs` → сними `isNewSong` с прошлой песни →
  `rebuild_config.py` + `deploy-miniapp.bat`.
- **`type: drama`** → нарезанные фразы дорамы (`video/<папка>/` + черновой
  videotheme): интегрируй в `config.json` секция `themes` → `rebuild_config.py` + деплой.

Сверяйся с чек-листом внутри самого сигнала — там точные занятые имена
(папка, videothemeNN, ключ config, обложка). После обработки перемести сигнал в
`agent_inbox/done/` и проставь `status: done`. Формат и правила —
`reports/agent_inbox/README.md`.
