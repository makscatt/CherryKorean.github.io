...
Этот репозиторий — **хранилище контента (CDN-склад)** для проекта
**Kimchi Hangugo**. Сюда складываются видео, картинки, JSON-данные тем,
конфиги. Активный фронтенд/бэкенд живёт в другом проекте и ссылается
на эти файлы по URL. Папка `_archive/` — устаревший фронтенд, для
понимания структуры (не править).

Известная нестыковка (сознательно игнорируется): часть картинок тем
фактически подтягивается из папки другого проекта, а не из этого
репо. К одному источнику пока не сведено. Не править без явного
указания босса.
